...
class StudentBulkImportTestCase(TestCase):
    ...
    def test_csv_bulk_import_63_students(self):
        ...
        # Verify 63 users + 1 existing user + 1 advisor user
        self.assertEqual(User.objects.filter(role='student').count(), 64)

    def test_excel_bulk_import_63_students(self):
        ...
        # Verify student users in DB
        self.assertTrue(User.objects.filter(username='xlsx_student_01').exists())
        self.assertTrue(User.objects.filter(username='xlsx_student_63').exists())
    ...
    def test_partial_failures_and_row_wise_errors(self):
        ...
        # Row 4: Completely empty row (should be skipped, not counted as failed)
        writer.writerow(['', '', '', '', '', ''])
        ...
        # Verify that only the valid student was created
        self.assertTrue(User.objects.filter(username='new_student_val').exists())
        self.assertFalse(User.objects.filter(username='new_student_val2').exists())
